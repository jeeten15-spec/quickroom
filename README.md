...
3. **Ads.** Google AdSense (`ca-pub-2208705874716134`) is in the HTML head plus `/ads.txt`. Manual IAB frames (728×90, stacked 160×600 on the left and right — three per side on Home/About/Blog/use cases/guides, two on shorter legal pages — plus 300×250 in-content and 320×50 on mobile) fill on public pages. Chat, Create, and the dashboard have no AdSense units. Keep Auto ads overlays/anchors/vignettes **off** in the AdSense dashboard so they cannot cover a room. Optional `VITE_ADSENSE_SLOT*` IDs improve fill. Monetag in-page push (3s delay) and vignette (10s, once per session) load only on articles/use cases — never on Home Create/Join or chat. Do not use OnClick/pop-under.
