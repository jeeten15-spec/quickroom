...
3. **Ads.** Use in-page display (AdSense) on landing, articles, and use cases only — never in rooms, and never Monetag OnClick/popunder tags (those intercept Create/Join clicks). `/sw.js` may still be hosted for Monetag verification. Set `VITE_ADSENSE_CLIENT` after AdSense approval.
