...
3. **Ads.** Google AdSense (`ca-pub-2208705874716134`) is in the HTML head plus `/ads.txt` on AdSense pages. Manual IAB frames: five 160×600 per side on `/blog`, four per side on use-case pages, three on Home/guides/articles, two on legal pages, plus 728×90 and 300×250. **About has no AdSense** — Monetag in-page push, vignette, and the sponsored link run only there. Chat, Create, and the dashboard have no ads. Keep Auto ads overlays/anchors/vignettes **off**. Optional `VITE_ADSENSE_SLOT*` IDs improve fill. Do not use OnClick/pop-under.
