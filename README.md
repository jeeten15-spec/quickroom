...
3. **Ads.** IAB frames (728×90, 160×600, 300×250, 320×50) are reserved on public pages. Set `VITE_ADSENSE_CLIENT` and `VITE_ADSENSE_SLOT` (optional size-specific slot vars) so those frames serve display ads (impressions). Monetag in-page push (3s delay) and vignette (10s, once per session) load only on articles/use cases — never on Home Create/Join or chat — because those formats overlay the page and cannot be clipped to a box. Do not use OnClick/pop-under.
