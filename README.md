...
3. **Ads.** Landing, articles, and use cases may show ads (Google AdSense and Monetag). Chat rooms do not. Monetag’s worker is served at `https://quickroom.org/sw.js`. Set `VITE_ADSENSE_CLIENT` after AdSense approval. In AdSense, enable **Privacy & messaging** for GDPR and US states. Then you may set `VITE_GOOGLE_FUNDING_CHOICES=true` so Google’s certified CMP is the EU banner.
