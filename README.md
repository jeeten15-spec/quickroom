...
3. **Ads.** Monetag **direct-link** units only (`omg10.com/4/11680015`), as labelled 728×90 / 160×600 / 320×50 slots on every public page and to the right of chat. They open in a new tab and never wrap Create/Join/room buttons. Do not use vignette, in-page push, OnClick, or pop-under. `/room/:id` is handled by a Pages Function that returns the SPA HTML with status 200 (do not use `room.html` — Pages pretty URLs 308 those paths and drop the room id).
