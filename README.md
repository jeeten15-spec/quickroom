...
3. **Ads.** Monetag in-page push (zone 11680018) and vignette (zone 11680014, 1s delay) tags go in the HTML `<head>` of public pages so the dashboard Verify button can see them. Homepage left/right 160×600 rails use the direct-link offer (`omg10.com/4/11680015`) until a dedicated Monetag banner zone exists. Chat rooms are served from `room.html` without those tags. Never use Monetag OnClick/popunder. Set `VITE_ADSENSE_CLIENT` after AdSense approval.
